...
type Package struct {
	...
	Symlinks        []string               `json:"symlinks,omitempty" yaml:"symlinks,omitempty"`                   // Glob patterns of paths in app-dir to symlink to bin-dir (directory mode only) - simple list format
	SymlinkPatterns map[string][]string    `json:"symlink_patterns,omitempty" yaml:"symlink_patterns,omitempty"`   // Platform-specific symlink patterns (e.g., {"darwin-*": ["bin/*"], "linux-*": ["lib/*"]})
