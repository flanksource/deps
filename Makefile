...
@for binary in $(2); do \
	...
	if [[ "$$artifact" == *.exe ]]; then \
		...
		cp "$$binary" ".release/$(1)"; \
		(cd .release && tar czf "$$archive_base.tar.gz" "$(1)" && $(SHA256) "$$archive_base.tar.gz" > "$$archive_base.tar.gz.sha256"); \
		rm -f ".release/$(1)"; \
	fi; \
done
endef
...
release: binaries
	mkdir -p .release
	rm -f .release/$(NAME)-linux-* .release/$(NAME)-darwin-* .release/$(NAME)-windows-* .release/$(NAME) .release/$(NAME).exe
	$(call archive_binaries,$(NAME),.bin/$(NAME)-linux-* .bin/$(NAME)-darwin-* .bin/$(NAME)-windows-*)

.PHONY: release-start
release-start: binaries-start
	mkdir -p .release
	rm -f .release/deps-start-*
	$(call archive_binaries,deps-start,.bin/deps-start-*)
